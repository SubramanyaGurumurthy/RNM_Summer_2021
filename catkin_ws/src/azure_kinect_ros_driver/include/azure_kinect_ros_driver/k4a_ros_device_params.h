...
#define ROS_PARAM_LIST                                                                                                 \
  ...
  LIST_ENTRY(subordinate_delay_off_master_usec,                                                                        \
              ...
              int, 0)                                                                                                  \
  LIST_ENTRY(calibration_file, "Path to a calibration file to use instead of the factory calibration", std::string,    \
              std::string(""))

class K4AROSDeviceParams
{
public:
  static void ReadFromFile(std::ifstream& fin, k4a::calibration& k4a_calibration);
...
private:
  static void ParseRows(const YAML::Node& node, k4a_calibration_extrinsics_t& calib);
  static void ParseIntrinsicParameters(const YAML::Node& node, k4a_calibration_intrinsics_t& calib);
};
