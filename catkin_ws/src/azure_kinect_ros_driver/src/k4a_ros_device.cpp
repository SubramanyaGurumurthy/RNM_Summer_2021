...
k4a_result_t K4AROSDevice::startCameras()
{
  ...
  if (k4a_device_)
  {
    ...
    if (!params_.calibration_file.empty()
      && boost::filesystem::is_regular_file(params_.calibration_file)
      && params_.calibration_file.size() >= suffix.size()
      && std::equal(suffix.rbegin(), suffix.rend(), params_.calibration_file.rbegin()))
    {
      // open supplied yaml file
      ROS_INFO("Testing configuration file");
      std::ifstream fin;
      fin.open(params_.calibration_file.c_str());
      if (!fin.good() || !fin.is_open()) {
        ROS_WARN("Failed to open calibration configuration file! Using factory calibration");
      } else
      {
        ROS_INFO("Loading calibration from config file");
        k4a::calibration k4a_calibration = k4a_device_.get_calibration(k4a_configuration.depth_mode, k4a_configuration.color_resolution);
        K4AROSDeviceParams::ReadFromFile(fin, k4a_calibration);
        calibration_data_.initialize(k4a_calibration, params_);
        from_file = true;
      }
    }
    if (!from_file)
    {
      ROS_INFO("Using factory calibration");
